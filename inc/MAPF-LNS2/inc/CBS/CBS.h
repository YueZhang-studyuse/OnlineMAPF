...
protected:
	...
	set<int> getInvalidAgents(const list<CBSConstraint>& constraints); // return agents that violates the constraints
	//conflicts
	void findConflicts(HLNode& curr);
	void findConflicts(HLNode& curr, int a1, int a2);
	shared_ptr<CBSConflict> chooseConflict(const HLNode &node) const;
	static void copyConflicts(const list<shared_ptr<CBSConflict>>& conflicts,
		list<shared_ptr<CBSConflict>>& copy, const list<int>& excluded_agent) ;
	void removeLowPriorityConflicts(list<shared_ptr<CBSConflict>>& conflicts) const;
	void computeSecondPriorityForConflict(CBSConflict& conflict, const HLNode& node);
	...
	void computeConflictPriority(shared_ptr<CBSConflict>& con, CBSNode& node); // check the conflict is cardinal, semi-cardinal or non-cardinal
