...
typedef std::tuple<int, int, int, int, constraint_type> CBSConstraint;
...
std::ostream& operator<<(std::ostream& os, const CBSConstraint& constraint);
...
public:
	...
	bool rectangleConflict(int a1, int a2, const std::pair<int, int>& Rs, const std::pair<int, int>& Rg,
	                         int Rg_t, const list<CBSConstraint>& constraint1, const list<CBSConstraint>& constraint2) // For RM
	{
		this->a1 = a1;
		this->a2 = a2;
...
std::ostream& operator<<(std::ostream& os, const CBSConflict& conflict);

bool operator < (const CBSConflict& conflict1, const CBSConflict& conflict2);
