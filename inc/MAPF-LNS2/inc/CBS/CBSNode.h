...
{
public:
	list<CBSConstraint> constraints; // new constraints
	...
	list<shared_ptr<CBSConflict> > conflicts;
	list<shared_ptr<CBSConflict> > unknownConf;

	// The chosen conflict
	shared_ptr<CBSConflict> conflict;
...
struct ConstraintsHasher // Hash a CT node by constraints on one agent
{
	...
	struct EqNode
	{
		bool operator() (const ConstraintsHasher& c1, const ConstraintsHasher& c2) const
		{
			...
			std::set<CBSConstraint> cons1, cons2;
			auto curr = c1.n;
