...
public:
	...
	shared_ptr<CBSConflict> run(const shared_ptr<CBSConflict>& conflict,
...
private:
	...
	shared_ptr<CBSConflict> findCorridorConflict(const shared_ptr<CBSConflict>& conflict,
		const vector<Path*>& paths, const HLNode& node);
	int findCorridor(const shared_ptr<CBSConflict>& conflict,
	...
	bool blocked(const Path& path, const CBSConstraint& constraint);
