...
// find path using traffic penalty
// Returns a path that minimizes the collisions with the paths in the path table, breaking ties by the length
Path SIPP::findPath(const ConstraintTable& constraint_table, unordered_map<pair<int,int>,int>& edges_count)
{
    reset();
    ...
    while (!focal_list.empty())
    {
        ...
        for (int next_location : instance.getNeighbors(curr->location)) // move to neighboring locations
        {
                    ...
                    curr->location, next_location, curr->timestep + 1, curr->high_expansion + 1))
            {
                ...
                //using the calculated g (penalty)
                ...
                int next_g = next_timestep;
                // if (instance.getDegree(next_location == 2))
                // {
                    if (edges_count.find(make_pair(curr->location,next_location)) != edges_count.end())
                    {
                        if (edges_count.find(make_pair(next_location,curr->location)) != edges_count.end() && edges_count[make_pair(next_location,curr->location)] > edges_count[make_pair(curr->location,next_location)])
                        {
                            //going against the flow
                            next_g += 10;
                        }
                    }
                    else if (edges_count.find(make_pair(next_location,curr->location)) != edges_count.end())
                    {
                        next_g += 10;
                    }
                //}
                auto next = new SIPPNode(next_location, next_g, next_h_val, curr, next_timestep,
                                         next_high_generation, next_high_expansion, next_v_collision, next_collisions);
                // try to retrieve it from the hash table
