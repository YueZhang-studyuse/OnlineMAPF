...
shared_ptr<CBSConflict> CorridorReasoning::run(const shared_ptr<CBSConflict>& conflict,
...
int CorridorReasoning::findCorridor(const shared_ptr<CBSConflict>& conflict,
...
shared_ptr<CBSConflict> CorridorReasoning::findCorridorConflict(const shared_ptr<CBSConflict>& conflict,
	const vector<Path*>& paths, const HLNode& node)
{
    ...
    if (abs(t3 - t4) <= corridor_length && t3_ > t3 && t4_ > t4)
    {
        ...
        shared_ptr<CBSConflict> corridor = make_shared<CBSConflict>();
...
bool CorridorReasoning::blocked(const Path& path, const CBSConstraint& constraint)
{
	int a, loc, t1, t2;
	constraint_type type;
