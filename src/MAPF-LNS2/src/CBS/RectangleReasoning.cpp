...
shared_ptr<CBSConflict> RectangleReasoning::run(const vector<Path*>& paths, int timestep,
...
shared_ptr<CBSConflict> RectangleReasoning::findRectangleConflictByGR(const vector<Path*>& paths, int timestep,
	int a1, int a2, const MDD* mdd1, const MDD* mdd2)
{
	assert(timestep > 0);
	...
	list<CBSConstraint> constraint1;
	list<CBSConstraint> constraint2;
	bool succ;
	...
	if (!blocked(*paths[a1], constraint1) || !blocked(*paths[a2], constraint2))
		return nullptr;
	auto rectangle = shared_ptr<CBSConflict>(new CBSConflict());
...
shared_ptr<CBSConflict> RectangleReasoning::findRectangleConflictByRM(const vector<Path*>& paths, int timestep,
        int a1, int a2, const MDD* mdd1, const MDD* mdd2)
{
    shared_ptr<CBSConflict> rectangle = nullptr;
                ...
                for (int t2_end : g2s)
                {
                    ...
                    if (new_type > type || (new_type == type && new_area > area))
                    {
                        ...
                        if (succ && blocked(*paths[a1], constraint1) && blocked(*paths[a2], constraint2))
                        {
							type = new_type;
							area = new_area;
							rectangle = shared_ptr<CBSConflict>(new CBSConflict());
							rectangle->rectangleConflict(a1, a2, Rs, Rg, Rg_t, constraint1, constraint2);
                            if (type == 2)
							{
								rectangle->priority = conflict_priority::CARDINAL;
...
bool RectangleReasoning::ExtractBarriers(const MDD& mdd, int loc, int timestep,
					int dir1, int dir2, int start, int goal, int start_time, list<CBSConstraint>& B)
{
	int num_barrier;
...
bool RectangleReasoning::isEntryBarrier(const CBSConstraint& b1, const CBSConstraint& b2, int dir1)
{
...
bool RectangleReasoning::isExitBarrier(const CBSConstraint& b1, const CBSConstraint& b2, int dir1)
{
...
pair<int, int> RectangleReasoning::getIntersection(const CBSConstraint& b1, const CBSConstraint& b2)
{
	pair<int, int> b1_l = instance.getCoordinate(get<1>(b1));
	pair<int, int> b1_u = instance.getCoordinate(get<2>(b1));
	pair<int, int> b2_l = instance.getCoordinate(get<1>(b2));
...
bool RectangleReasoning::isCut(const CBSConstraint b, const pair<int, int>& Rs, const pair<int, int>& Rg)
{
	...
	for (const auto& b1_entry : B1)
	{
		for (const auto& b2_entry : B2)
		{
			if (isEntryBarrier(b1_entry, b2_entry, dir1) && isEntryBarrier(b2_entry, b1_entry, dir2))
			{
				...
				list<CBSConstraint>::const_reverse_iterator  b1_exit = B1.rbegin();
				list<CBSConstraint>::const_reverse_iterator  b2_exit = B2.rbegin();
...
bool RectangleReasoning::addModifiedBarrierConstraints(int a1, int a2, 
	...
	const MDD* mdd1, const MDD* mdd2,
	list<CBSConstraint>& constraint1, list<CBSConstraint>& constraint2)
{
...
bool RectangleReasoning::blocked(const Path& path, const list<CBSConstraint>& constraints)
{
