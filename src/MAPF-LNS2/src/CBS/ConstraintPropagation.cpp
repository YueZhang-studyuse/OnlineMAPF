...
std::pair<std::vector<CBSConstraint>, std::vector<CBSConstraint>> ConstraintPropagation::generate_constraints(int level_0, int level_1){
  MDD* mdd_s = mdd0;
  MDD* mdd_l = mdd1;
  bool reversed = false;
  if (level_0 > level_1){
  ...
  if (!non_mutexed.empty()){
    ...
    CBSConstraint length_con(0, goal_ptr_i->location, -1, level_0 - 1, constraint_type::GLENGTH);
    // std::pair<int, int> length_con = {-1, level_0};

    std::vector<CBSConstraint>cons_vec_1;
    for (auto& it:cons_set_1){
      cons_vec_1.push_back(CBSConstraint(1, it.second, -1, it.first, constraint_type::VERTEX));
    }
    // std::vector<std::pair<int, int>> cons_vec_1(cons_set_1.begin(), cons_set_1.end());
    cons_vec_1.push_back(CBSConstraint(0, goal_ptr_i->location,  -1, level_0 - 1, constraint_type::LEQLENGTH));
  ...
  std::vector<CBSConstraint> cons_vec_0;
  std::vector<CBSConstraint> cons_vec_1;
