...
void GCBS::copyConflicts(const list<shared_ptr<CBSConflict >>& conflicts,
...
shared_ptr<CBSConflict> GCBS::chooseConflict(const GCBSNode &node) const
{
    if (screen == 3)
        printConflicts(node);
...
set<int> GCBS::getInvalidAgents(const list<CBSConstraint>& constraints)  // return agents that violates the constraints
{
    set<int> agents;
    int agent, x, y, t;
