...
/*void CBS::copyConflicts(const list<shared_ptr<CBSConflict>>& conflicts,
	list<shared_ptr<CBSConflict>>& copy, int excluded_agent) const
{
...
void CBS::copyConflicts(const list<shared_ptr<CBSConflict >>& conflicts,
	list<shared_ptr<CBSConflict>>& copy, const list<int>& excluded_agents)
{
...
void CBS::findConflicts(HLNode& curr, int a1, int a2)
{
	...
	for (int timestep = 0; timestep < min_path_length; timestep++)
	{
		...
		if (loc1 == loc2)
		{
			shared_ptr<CBSConflict> conflict(new CBSConflict());
...
shared_ptr<CBSConflict> CBS::chooseConflict(const HLNode &node) const
{
	if (screen == 3)
		printConflicts(node);
	shared_ptr<CBSConflict> choose;
...
void CBS::computeSecondPriorityForConflict(CBSConflict& conflict, const HLNode& node)
{
	int count[2] = {0, 0};
...
void CBS::computeConflictPriority(shared_ptr<CBSConflict>& con, CBSNode& node)
{
...
void CBS::classifyConflicts(CBSNode &node)
{
	...
	{
		shared_ptr<CBSConflict> con = node.unknownConf.front();
		int a1 = con->a1, a2 = con->a2;
		int timestep = get<3>(con->constraint1.back());
		constraint_type type = get<4>(con->constraint1.back());
...
void CBS::removeLowPriorityConflicts(list<shared_ptr<CBSConflict>>& conflicts) const
{
	if (conflicts.empty())
		return;
	unordered_map<int, shared_ptr<CBSConflict> > keep;
	list<shared_ptr<CBSConflict>> to_delete;
	for (const auto& conflict : conflicts)
	{
...
set<int> CBS::getInvalidAgents(const list<CBSConstraint>& constraints)  // return agents that violates the constraints
{
	set<int> agents;
	int agent, x, y, t;
	constraint_type type;
