...
shared_ptr<CBSConflict> MutexReasoning::run(int a1, int a2, CBSNode& node, MDD* mdd_1, MDD* mdd_2)
{
	clock_t t = clock();
...
shared_ptr<CBSConflict> MutexReasoning::findMutexConflict(int a1, int a2, CBSNode& node, MDD* mdd_1, MDD* mdd_2){
  ...
  shared_ptr<CBSConflict> mutex_conflict = nullptr;
  ...
  if (mutex_conflict == nullptr){
    // generate constraint;
    mutex_conflict = make_shared<CBSConflict>();
  ...
  // prepare for return
  shared_ptr<CBSConflict> conflict_to_return = make_shared<CBSConflict>(*mutex_conflict);
